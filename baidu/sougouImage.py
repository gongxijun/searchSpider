...
class SougouSpider:
    ...
    def __getImg(self, sub_dest_dir, word='dame', extra_word='provice'):
        ...
        while pn <= self.__amount and self.__counter <= self.__amount:
            ...
            try:
                ...
                # 解析json
                # print data
                self.__saveImage(root_path, self.parseJson(data, 'thumbUrl'), word)
                pn += maxImageNum
    ...
    def parseJson(self, data, __key='objURL'):
        img_list = []
        _indx = 0;
        while len(data) > 2 and _indx != -1:
            _indx = data.find(__key);
            data = data[_indx:];
...
def total_main():
    ...
    while (_index < args.stride):
        spider = SougouSpider(image_path=args.dest_dir)
...
if __name__ == '__main__':
    #total_main()
    args = parse_args()
    spider = SougouSpider(image_path='.')
